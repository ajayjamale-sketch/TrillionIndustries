...
const INITIAL_MATRIX: Record<string, Record<string, boolean[]>> = {
  ...
  'Warehouse Manager': { Production: [true, false, false, false, false], Procurement: [true, false, false, false, false], Inventory: [true, true, true, false, false], Finance: [true, false, false, false, false], Analytics: [true, false, false, false, false] },
  'Maintenance Engineer': { Production: [true, false, false, false, false], Procurement: [true, false, false, false, false], Inventory: [true, false, false, false, false], Finance: [true, false, false, false, false], Analytics: [true, false, false, false, false] },
  'Quality Manager': { Production: [true, false, false, false, false], Procurement: [true, false, false, false, false], Inventory: [true, false, false, false, false], Finance: [true, false, false, false, false], Analytics: [true, false, false, false, false] },
  'Supplier': { Production: [false, false, false, false, false], Procurement: [true, false, false, false, false], Inventory: [true, false, false, false, false], Finance: [false, false, false, false, false], Analytics: [false, false, false, false, false] },
};
...
export function AccessControlPage({ user }: { user: User }) {
  ...
  const togglePermission = (module: string, actionIndex: number) => {
    setMatrix(prev => {
      const roleMatrix = prev[selectedRole] || {
        Production: [false, false, false, false, false],
        Procurement: [false, false, false, false, false],
        Inventory: [false, false, false, false, false],
        Finance: [false, false, false, false, false],
        Analytics: [false, false, false, false, false],
      };
      
      const modulePermissions = [...(roleMatrix[module] || [false, false, false, false, false])];
      modulePermissions[actionIndex] = !modulePermissions[actionIndex];
  ...
  const handleSaveChanges = () => {
    toast.success(`RBAC configuration for ${selectedRole} saved successfully!`);
  };
  ...
  return (
    <div className="p-6 space-y-5 max-w-5xl">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-0.5">Manage role-based permissions across all modules</p>
      </div>
      ...
      <div className="grid lg:grid-cols-4 gap-5">
          ...
          <div className="divide-y divide-border">
            {ROLES.map(role => (
              <button 
                ...
                className={`w-full px-4 py-3 text-xs text-left font-medium transition-colors ${selectedRole === role ? 'bg-primary/10 text-primary' : 'text-muted-foreground hover:bg-muted/50 hover:text-foreground'}`}
              >
                {role}
              </button>
            ))}
        ...
        <div className="lg:col-span-3 bg-card border border-border rounded-xl overflow-hidden">
          <div className="px-5 py-4 border-b border-border flex items-center justify-between">
            ...
            <button onClick={handleSaveChanges} className="px-3 py-1.5 rounded-xl bg-primary text-primary-foreground text-xs font-semibold hover:bg-primary/90 transition-colors">
              Save Changes
            </button>
          </div>
              ...
              <thead className="bg-muted/50 text-xs text-muted-foreground">
                <tr>
                  ...
                  {['View', 'Create', 'Edit', 'Delete', 'Approve'].map(a => (
                    <th key={a} className="text-center px-3 py-3 font-medium">{a}</th>
                  ))}
                </tr>
              </thead>
                    ...
                    <tr key={p.module} className="hover:bg-muted/30 transition-colors">
                      ...
                      {perms.map((allowed, i) => (
                        <td key={i} className="px-3 py-3 text-center">
                          <button 
                            onClick={() => togglePermission(p.module, i)}
                            ...
                            title={`Toggle ${p.actions[i]} for ${p.module}`}
                          >
